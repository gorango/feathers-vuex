...
export const initAuth = async function initAuth (options) {
  const { commit, dispatch, req, moduleName, cookieName } = Object.assign({}, authDefaults, options)
  ...
  try {
    const user = await dispatch(moduleName + '/populateUser', payload.userId)
